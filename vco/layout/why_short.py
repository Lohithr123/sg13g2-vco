...
l2n.extract_netlist()
n=l2n.probe_net(l2n.layer_by_name("M5"), pya.DPoint(-145.0,-95.23))
print("outbp net:", n.expanded_name() if n else "none")
for k in ("M5","TM1","M2","M1"):
    r=l2n.shapes_of_net(n, l2n.layer_by_name(k), True)
    got=[p.bbox().to_dtype(ly.dbu) for p in r.each()]
    wide=[b for b in got if b.left < -20 and b.right > 20]
    if wide:
        print(f"  {k}: shapes crossing the centre:")
        for b in wide[:5]:
