...
l2n.extract_netlist()
n = l2n.probe_net(l2n.layer_by_name("M1"), pya.DPoint(-8.51, -108.0))
print("XSW0 A-side pin net:", n.expanded_name() if n else "none")
W = pya.DBox(-30, -115, 0, -95)
for k in ("M3","M4","M5","TM1"):
    r = l2n.shapes_of_net(n, l2n.layer_by_name(k))
    got = [p.bbox().to_dtype(ly.dbu) for p in r.each()]
    got = [b for b in got if b.overlaps(W)]
    if got:
        print(f"\n  {k}:")
        for b in sorted(got, key=lambda q: -(q.width()*q.height()))[:5]:
